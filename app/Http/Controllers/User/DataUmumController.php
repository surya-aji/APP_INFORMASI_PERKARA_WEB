...
class DataUmumController extends Controller
{
    public function index(){
        ...
        $kuasa = DB::connection('mysql')->table('perkara_pengacara')
        ->select('perkara_pengacara.nama as kuasa_pemohon')
        ->where('pihak_ke',1)
        ->where('perkara_id',$url)
        ->get();
        
        $kuasa2 = DB::connection('mysql')->table('perkara_pengacara')
        ...
        $datasidang = DB::connection('mysql')->table('perkara_jadwal_sidang')
        ->latest('tanggal_sidang')
        ->where('perkara_id',$url)
        ->first();

        $jadwal_agenda = DB::connection('mysql')->table('perkara_jadwal_sidang')
